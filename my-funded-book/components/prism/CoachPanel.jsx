  ...
  return (
    <section className="mb-4 overflow-hidden rounded-2xl border border-line bg-panel">
      <div className="flex items-center justify-between border-b border-line px-4 py-3">
        <div className="flex items-center gap-2"><Brain size={17} className="text-accent" /><div><h3 className="text-[13px] font-bold text-white">PRISM Coach</h3><p className="text-[10px] text-muted2">Assistant IA de ton journal de trading</p></div></div>
        <span className="rounded-full border border-line px-2 py-1 font-mono text-[9px] tracking-wider text-muted2">EN LIGNE</span>
      </div>
      <div className="min-h-52 space-y-3 p-4">
        ...
        </> : messages.map((message, index) => <div key={`${message.role}-${index}`} className={`max-w-[88%] rounded-xl px-3 py-2 text-[12px] leading-relaxed ${message.role === "user" ? "ml-auto bg-white text-black" : "border border-line bg-panel2 text-muted2"}`}><MessageText>{message.content}</MessageText></div>)}
