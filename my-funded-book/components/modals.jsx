...
export function SettingsModal({ onClose }) {
  ...
  const [f, setF] = useState({ name: profile.name, pin: profile.pin, starting_balance: profile.starting_balance });
  const set = (k, v) => setF((s) => ({ ...s, [k]: v }));
  ...
  return (
    <Modal title="Réglages" onClose={onClose}
      footer={<><GhostBtn className="flex-1" onClick={exportCSV}>Export CSV</GhostBtn><PrimaryBtn className="flex-1" onClick={async () => { await saveProfile({ name: f.name || "trader", pin: f.pin || "1234", starting_balance: Number(f.starting_balance) || 0 }); onClose(); }}>Enregistrer</PrimaryBtn></>}>
      ...
      <Field label="Balance de départ ($)"><input type="number" className={inputCls} value={f.starting_balance} onChange={(e) => set("starting_balance", e.target.value)} /></Field>
    </Modal>
  );
}
