...
export async function POST(request) {
  ...
  const response = await fetch("https://api.groq.com/openai/v1/chat/completions", { method: "POST", headers: { "Content-Type": "application/json", Authorization: `Bearer ${process.env.GROQ_API_KEY}` }, body: JSON.stringify({ model: "openai/gpt-oss-20b", max_tokens: 700, temperature: 0.55, messages: [{ role: "system", content: `Tu t'appelles PRISM. Tu es le coach intégré de MyTradeBook. Reste bref, concret et n'offre jamais de conseil financier ni de signal. Journal actuel: ${list.length} trades, P&L ${net}$, ${totalR}R.` }, ...history, { role: "user", content: message }] }) });
