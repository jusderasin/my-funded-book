          ...
          {rows.map((r, i) => {
            ...
            return (
              <div key={r.user_id} className={`flex items-center gap-3 px-4 py-3 ${i > 0 ? "border-t border-line" : ""} ${me ? "bg-accentDim" : ""}`}>
                ...
                <span className={`w-20 text-right font-mono text-[13px] font-bold ${r.total_r >= 0 ? "text-accent" : "text-loss"}`}>{fmtR(r.total_r)}</span>
                <span className={`w-24 text-right font-mono text-[12px] ${r.total_pnl >= 0 ? "text-muted" : "text-loss"}`}>{fmtPnl(r.total_pnl)}</span>
              </div>
            );
          })}
        </div>
